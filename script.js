...
function generateRandomString(length) {
	...
	for (let i = 0; i < length; i++) {
		...
		// EXTRACT THE CHARACTER AT THE RANDOM INDEX AND APPEND IT TO THE RESULT
...
// THERE REAL FUNCTION FOR SENDING AN HTTP REQUEST
async function _sendRequest(url, data) {
	try {
...
// A MOCK FUNCTION THAT RETURNS HARD DATA
// USED ONLY FOR SPEED IN DEVELOPMENT
...
document.addEventListener("DOMContentLoaded", () => {
	...
	const signupData = {
		...
		passwordConfirmation: "@passWORD2024",
	...
	const apiUrl = "http://localhost:8000/api/authenticator/users/create/"

	sendRequest(apiUrl, signupData)
		.then(data => displayResponse(JSON.parse(data).resume))
		.catch(error => console.error("Error displaying response:", error))
})
